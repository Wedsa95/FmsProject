            ...
            if (id) {
                this.phoneNumberService.find(id)
                    .subscribe((phoneNumberResponse: HttpResponse<PhoneNumber>) => {
                        const phoneNumber: PhoneNumber = phoneNumberResponse.body;
                        this.ngbModalRef = this.phoneNumberModalRef(component, phoneNumber);
                        resolve(this.ngbModalRef);
                    });
            } else {
                // setTimeout used as a workaround for getting ExpressionChangedAfterItHasBeenCheckedError
                setTimeout(() => {
                    this.ngbModalRef = this.phoneNumberModalRef(component, new PhoneNumber());
                    resolve(this.ngbModalRef);
                }, 0);
            }
        });
